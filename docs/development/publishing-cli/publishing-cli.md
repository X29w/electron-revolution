# @x-elevolution/cli — Documentation

Complete documentation for the `@x-elevolution/cli` scaffolding and code generation tool.

**Version:** 0.2.0  
**License:** MIT  
**Install:** `npm install -g @x-elevolution/cli` or use via `npx @x-elevolution/cli`
...
`@x-elevolution/cli` is a code generation tool that:

1. **Scaffolds complete Electron projects** with the X-Elevolution architecture
...
**Flags:**
- `--local` — Link `@x-elevolution/core` to the local monorepo path instead of the npm version. Used for development.
...
- `@x-elevolution/core` added as dependency (`^0.2.0` or local link)
...
3. Test with `x-elevolution create test-project --local`
...
- Sets `@x-elevolution/core` dependency to `^0.2.0` (or local link with `--local`)
...
case "my-thing":
  if (!subCommand) {
    console.error("Usage: x-elevolution my-thing <name>");
    process.exit(1);
  }
...
```ts
console.log(`    ${c.cyan}x-elevolution my-thing${c.reset} <name>    Add a my-thing`);
```
